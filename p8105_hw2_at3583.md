...
# Summaries for questions

prof_total_weight \<- trash_df %\>% filter(wheel == “Professor Trash
Wheel”) %\>% summarise(total_weight = sum(weight_tons, na.rm = TRUE))

gwynnda_cigs \<- trash_df %\>% filter(wheel == “Gwynnda”, month ==
“June”, year == 2022) %\>% summarise(total_cigs = sum(cigarette_butts,
na.rm = TRUE)) \`\`\`
